...
final readonly class AttachDocument implements OwnedDocuments
{
    ...
    public function assertCleanOwned(array $documentIds, string $ownerId, array $requiredPurposes): array
    {
        ...
        foreach ($documentIds as $index => $documentId) {
            ...
            if (! $purpose instanceof DocumentPurpose) {
                throw new InvalidArgumentException;
            }
        ...
        $references = [];
        $now = $this->clock->now();
        foreach ($requiredById as $documentId => $requiredPurpose) {
            ...
            if ($row->purpose !== $requiredPurpose->value) {
